...
public class TestVariant3 {
    ...
    public void loginOld() {
        assertEquals(new Variant3().booleanTask(3), false);
    }

    @Test(dataProvider = "inputProvider")
    public void inputTest(int p1, long p3) {
        assertEquals(new Variant3().inputOutputTask(p1), p3);
    }
    ...
    public void negativeInputTest() {
        new Variant3().inputOutputTask(-2);
    }

    @Test(dataProvider = "integerProvider")
    public void integerTest(int p1, int p3) {
        assertEquals(new Variant3().integerNumbersTask(p1), p3);
    }
    ...
    public void negativeIntegerTest() {
        new Variant3().integerNumbersTask(-2);
    }
    ...
    public void switchTest(int p1, SEASON p2) {
        assertEquals(new Variant3().switchTask(p1), p2);
    }

    @DataProvider
    public Object[][] switchProvider() {
        return new Object[][]{{3, SEASON.SPRING}, {6, SEASON.AUTUMN}};
    }

    @Test(expectedExceptions = AssertionError.class)
    public void switchNegativeTest() {
        new Variant3().switchTask(10);
    }

    @Test(dataProvider = "forProvider")
    public void forTest(int n, double p2) {
        assertEquals(new Variant3().forTask(n), p2, EPS);
    }
    ...
    public void whileTest(int a, int b, int c) {
        assertEquals(new Variant3().whileTask(a, b), c);
    }
    ...
    public void arrayTest(double[] array, double value) {
        assertEquals(new Variant3().arrayTask(array), value);
    }

    @DataProvider
    public Object[][] arrayProvider() {
        return new Object[][]{{new double[]{10, 2, 3}, 0}, {new double[]{10, 2, 13}, 10}, {new double[]{4, 3, 5, -4, 9, 2}, -4}};
    }

    @Test(dataProvider = "matrixProvider")
    public void twoDimensionArrayTest(int[][] input, int from, int to, int[][] output) {
        assertEquals(new Variant3().twoDimensionArrayTask(input, from, to), output);
    }
